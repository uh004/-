# Visualization에 가구당 평균 가격 추이, 가구당 거래 건수 추이, 지역별 평군 가격 막대 그래프
...
def barChart(total_df):
  ...
  bar_df2 = result.groupby('SGG_NM')['REQ_GBN'].agg('count').reset_index()

  df_sorted = bar_df.sort_values('OBJ_AMT', ascending=False)
  df_sorted2 = bar_df2.sort_values('REQ_GBN', ascending=False)

  # 바 차트 만들기
  fig = px.bar(df_sorted, x='SGG_NM', y='OBJ_AMT')
  fig2 = px.bar(df_sorted2, x='SGG_NM', y='REQ_GBN')
  ...
  fig2.update_yaxes(tickformat='.0f',
                  title_text = '거래 건수(건)',
                  range=[0, df_sorted2['REQ_GBN'].max()])
  fig2.update_layout(title='Bar Chart - 내림차순',
                    xaxis_title='지역구명',
                    yaxis_title='거래건수(건)')
